    ...
    <div className="panel" aria-label="Nearby users" style={{ display: 'grid', gap: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p className="page-subtitle" style={{ margin: 0 }}>Sorted by proximity (mock liveness).</p>
        </div>
        <Button
          variant="ghost"
        ...
        {nearby.map((user, idx) => (
          <div key={user.id} className="route-chip" data-testid={`nearby-${idx}`} style={{ textAlign: 'left' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span className="badge">{user.avatarPreset}</span>
            </div>
            <div style={{ color: 'var(--color-text-muted)', fontSize: '12px' }}>
              {user.sharedInterests.join(', ') || 'No shared interests yet'}
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span aria-label={`Distance to ${user.displayName}`}>
